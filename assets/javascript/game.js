...
//this function will execute as soon as anything with the characters class is selected. interestingly enough though, although you remove the class "characters"(within the codeblock of this functino) from any of the elements that might trigger this function on click, they will still trigger the function the 2+ nth time they are clicked. in order to resolve that I had to use if statements and booleans.
$(".characters").click(function(){//see comment above^^^
	//this if clause will only go if the the game has not been deemed over and if the initial character has not yet been chosen
	if(!hasCharacterBeenChosen && !gameOver){//see comment above^^^
	//since this will only run when the character initially gets selected, it makes sense to remove it as a character option since it will not be part of the enemies group
	$(this).removeClass("characters");//see comment above^^^
	//since the players choice has been removed from the characters class, now I can place everyone left in the characters class in the pending enemies section of the page
	$ ("#enemies").append($(".characters"));//see comment above^^^
	//I will need to refer back to the character that was chosen later, in the interest of being able to access all the information in that particular element, I save it below in jquery object form under the characterChosen variable
	characterChosen = $(this);//see comment above^^^
	//here we are assigning the value of whatever is in the value attribute of the element selected that triggered this on click event character to our yourhp variable
	...
	//now this one is a little different, your attack is constantly going to be changing, so what we need is to store the original attack value so that only that will be added every click
	attackAdder = $(this).data("attack");//see comment above^^^
	//our enemies array can only ever comprise of the enemies pending to be selected, since the character chosen doesn't logically fall under that category, we have to remove it by using the javascript splicemethod below, in order to know what index to remove we take advantage of the fact that the id values, in this case are the exact same as the values inside the enemies array. this way we can use the indexOf method to pull up the value of the element being selected, the second argument in the enemies.splice call is the amount of values that we want removed.
	enemies.splice(enemies.indexOf($(this).attr("id")), 1);//see comment above^^^
	//finally, we have to tell the program to display all these values that we spent the last couple of lines storing and manipulating, you can find a more comprehensive explanation of the displayyourvalues function on line 148.
	displayyourvalues();//see comment above^^^
	//with this boolean we are storing the fact that the players character has been chosen as true.
	hasCharacterBeenChosen = true;//see comment above^^^
	//I am creating a variable in the local scope because I never use this variable outside the context of this function so it is unnecessary to generate a global variable. what I am assigning to it is the value of the id attribute for the element being selected and concatenating a period(.) so that the end result is a class representation with that same text
 	var characterChosenForRound = "."+ characterChosen.attr("id");//see comment above^^^
 	//now I use that class representation in order to select that particular class, unique to one element on the body, and give it the css value of inline-block, which in effect will make it visible, since prior to this it had display:none as its css display property. I use the css method instead of show because I do not want it to go back to block, because then the two characters, one enemy, one main player will not be next to one another. 
 	$(characterChosenForRound).css("display","inline-block");//see comment above^^^
 	//I want it to be known that the character chosem will always be fighting someone else, to signal that I added a text statement to the right of the chosen character of the player. 
...
//this else if will only run if the players character has already been chosen and the game is not over
	...
	//refer to line 180 for a more thorough explanation of what is being done below, only difference is that it is being done because an enemy is moving from pending to currently in fight.
	enemies.splice(enemies.indexOf($(this).attr("id")), 1);//see comment above^^^
	...
	//as soon as an enemy is chosen, that defacto means that a match is about to start between the user and the enemy that has moved from pending to fighting mode. in order to ensure that no other character can be chosen and thereby execute this code block again, I change the boolean value of roundHasBegun to true. 
...
//we only want the attack to be executed if the game isn't over already and the actual fighting round has begun, meaning there is someone in the defender position and someone in the offensive position. those are the reasons for the two conditions below. 
...
//your attack power has to increase every time attack, but it has to happen after the actual attack , otherwise the enemy would never receive a damage amount that equals to the users base attack power, it will start at that attack power plus itself.
